...
export function move(toX, toY) {
    ...
    else if (toDelete2.length > 0 && option2[0] === toX && option2[1] === toY) { newBoard = newBoard.filter((piece) => toDelete2[3] !== piece[3]) }
    else if (toDelete3.length > 0 && option3[0] === toX && option3[1] === toY) { newBoard = newBoard.filter((piece) => toDelete3[3] !== piece[3]) }
    else if (toDelete4.length > 0 && option4[0] === toX && option4[1] === toY) { newBoard = newBoard.filter((piece) => toDelete4[3] !== piece[3]) }

    newBoard.push(newPiecePosition)
    // console.log(toDelete1, toDelete2, toDelete3, toDelete4, option1, option2, option3, option4)
